...
import { validateCreateNodeInput, validateBulkNodes } from '@/lib/api/validation';
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const validation = validateCreateNodeInput(body);
    ...
    const position = validation.data.position ?? existingNodes.length;
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const validation = validateBulkNodes(body);
    if (!validation.valid) return errorResponse(validation.error, 400);
    ...
    // Transform validated nodes to create input format
    const nodesToCreate = validation.data.nodes.map((node, index) => ({
      type: node.type,
